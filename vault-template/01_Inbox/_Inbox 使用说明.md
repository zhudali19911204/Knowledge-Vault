...
“知识收”只转换文件并创建尽量完整的来源笔记，不选择精简/保真模式；“知识理”同样不分精简/保真，只询问由用户自定义卡片数量与内容，还是由 AI 推荐后直接批处理。高置信度卡片进入 `02_Domains` 至 `05_Skills`；只有本轮全部卡片成功路由，来源才进入 `06_Archive`，存在待审卡或路由失败时来源继续留在 Inbox。原图模式的图片统一放入 `07_Attachments`。
