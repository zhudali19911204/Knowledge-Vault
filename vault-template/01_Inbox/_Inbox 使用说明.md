...
“知识收”只转换文件并创建尽量完整的来源笔记，不选择精简/保真模式；“知识理”同样不分精简/保真，只询问由用户自定义卡片数量与内容，还是由 AI 推荐后直接处理。高置信度卡片进入 `02_Domains` 至 `05_Skills`，处理后的来源进入 `06_Archive`，原图模式的图片统一放入 `07_Attachments`。
