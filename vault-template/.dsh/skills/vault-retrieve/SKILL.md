...
7. 回答列出实际使用的知识库文档，区分“知识库已有结论”和“AI 补充说明”。回答末尾使用 `## 来源与关联`，其中来源笔记、所属索引和相关知识统一输出为 `[[Vault相对路径（不含 .md）|标题]]`，与 YAML `related` 相同；每个目标必须是实际读取并确认存在的 Vault 文件，禁止只写普通标题、`file:///` 或磁盘绝对路径。Harness 会把这种双链转换为可点击的内置阅读器链接。
