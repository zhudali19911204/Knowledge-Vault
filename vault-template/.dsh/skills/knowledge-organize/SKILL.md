...
- 第 2 步写入前静默检查一次，不输出检查过程：每张卡只回答一个稳定问题；卡间结论和边界不重复；结论与正文均由 `evidence` 支持；`triggers/use/avoid/questions` 足以独立检索。不合格时先在内存中合并、拆分或改写，再一次性写入。
- SOP 不按步骤拆散,每一步有图片必须放对应的图片；紧凑契约中的 `kind` 使用 `procedure`，`body` 完整保留前置条件、输入、步骤、参数、验证、异常、注意事项和实际存在的 `![[07_Attachments/...]]`。
- 不为凑数创建空卡或重复卡。`aliases`、`related` 没有真实内容时省略；跨主题关联交给“知识联”。
