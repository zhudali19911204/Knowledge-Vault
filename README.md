...
`知识理` 按来源逐篇批处理，正常路径固定为三次文件工具调用：`prepare` 一次返回带证据标记的完整来源、现有目录与收录范围、主题归属偏好和紧凑契约，模型直接写一次 `cards.json`，随后 `apply --cleanup` 一次完成预检、渲染、反链、索引、定向路由和终检。流程禁止临时 builder 和重复打开来源或产物。存在 `needs-review` 卡片或路由失败时，来源继续留在 Inbox；全部卡片成功路由后才归档来源。

自动入库先判断每张卡片实际解决的问题，再选择目录。同主题概念与 SOP 优先集中，已有主题的位置优先于文档类型的默认分类；标题和背景关键词不会强制改变落点。只有现有目录不合适且形成稳定子主题时才新建，并自动分配连续编号。例如一篇标题提到 LLM、正文讲 Agent 的 ReAct 方法的文章，在已有或已确认的 LLM → Agent 分类下入库，不因 LLM 字样在另一大类重复建目录。

新主题的大类或具体位置有歧义时，Agent 在写入前集中询问一次；未回答或暂缓时本批保持待处理。用户明确确认的主题归属会保存在 Vault 的 `.agents/routing-preferences.json`，后续子主题沿用。完成后展示每张卡片的最终目录和简短理由。此更新随模板初始化生效，已有 Vault 不会自动同步新脚本或规则。
...
编号是强制的层级编码：每一级在父编号后追加两位，从 `01` 开始按现有最大编号顺序递增。例如 `02_Domains` 下依次为 `0201_xxx`、`0202_xxx`，`0201_xxx` 的子目录依次为 `020101_xxx`、`020102_xxx`。已有目录填写真实完整路径，新建部分只填写语义名称，由脚本分配编号；不得跳号、重用已删除编号，或继续使用无编号及编号层级错误的目录。

“知识理”的准备阶段提供 `02_Domains` 至 `05_Skills` 的目录上下文，模型先判断卡片主主题，再结合 `_Index.md` 的收录范围和用户确认的主题归属选择位置。`title`、`aliases`、`triggers` 用于理解跨语言与同义词，不独立决定落点。路由器执行已经选定的完整路径，预检整批目标并为新目录统一编号，不按文件名覆盖语义选择；`06_Archive` 仅用于来源归档。

可单独运行 `python -B Test-KnowledgeRouting.py` 验证路由、编号、歧义保护和重试；`Test-KnowledgeBase.ps1` 也会执行这些测试。自动测试使用临时 Vault 和预设的卡片分类，不调用模型；实际语义分类效果仍需在聊天整理流程中验证。
